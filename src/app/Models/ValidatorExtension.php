...
class ValidatorExtension extends Validator
{
    ...
    public function validateHasChild($attribute, $value, $parameters)
    {
        $deleted_ip_column = 'deleted_ip';
        $deleted_ip = array_get($this->data, $deleted_ip_column);
        if ($deleted_ip === null) {
            return true;
        }
        $parameters = array_map('trim', $parameters);
        ...
        $extra = array();
        $ignore_column = 'id';
        $ignore_id = array_get($this->data, $ignore_column);
        foreach ($parameters as $parameter) {
            $parameter = array_map('trim', explode('=', $parameter, 2));
            $field_name = $parameter[0];
            if (count($parameter) > 1) {
            ...
            if ($field_name == $column) {
                ...
                if (array_get($this->data, $field_name) !== 0) {
                    $extra[$column_name] = array_get($this->data, $field_name);
                }
            }
        }
